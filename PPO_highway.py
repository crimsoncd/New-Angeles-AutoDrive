...
class PPO:
    ...
    def take_action(self, state):  # 输入当前时刻的状态
        ...
        actions = action_dict.sample().tolist()
    
        action = actions[random.randint(0,120)]
    ...
    def learn(self, transition_dict):
        ...
        next_states_target = self.critic(next_states).view(self.epochs,-1)
        # print(next_states_target.size())
        ...
        td_value = self.critic(states).view(self.epochs,-1)
        # 时序差分，预测值-目标值  # [b,1]
        ...
        # numpy --> tensor
        advantage = torch.tensor(advantage_list, dtype=torch.float).repeat(2,1).to(self.device)
        ...
        old_log_prob = torch.zeros_like(mu)
        for i in actions:
            i = torch.tensor(i,dtype=torch.float).to(self.device)
            log = action_dists.log_prob(i)
            old_log_prob += log

        old_log_prob /= self.epochs
        # print(old_log_prob.size())
        ...
        for _ in range(self.epochs):
            ...
            log_prob /= self.epochs
            
            # 计算概率的比值来控制新策略更新幅度
            ratio = torch.exp(log_prob - old_log_prob).view(2*self.epochs,-1)
            
            # print(ratio.size(),advantage.size())
            # 公式的左侧项
            ...
            # 策略网络的损失PPO-clip
            actor_loss = torch.mean(-torch.min(surr1,surr2)).to(self.device)
            # print(actor_loss)
            # 价值网络的当前时刻预测值，与目标价值网络当前时刻的state_value之差
            # print(self.critic(states).size(),td_target.size())
            critic_loss = torch.mean(F.mse_loss(self.critic(states), td_target.view(-1,1).detach()))
