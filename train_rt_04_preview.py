...
def parse_args():
    ...
    parser.add_argument("--env-id", type=str, default="Racetrack_train_02",
...
def WriteServeral(folder, v_loss, pg_loss, e_loss):
...
if __name__ == "__main__":
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_highway_env(args.seed + i) for i in range(args.num_envs)]
    )
    ...
    for update in range(1, num_updates + 1):
        ...
        print("Update", update, "out of", num_updates+1, end=", ")
        if args.anneal_lr:
        ...
        for step in range(0, args.num_steps):
            ...
            next_obs,reward,done,trunctions,info = envs.step(action.cpu().numpy())
        ...
        WriteServeral(task_folder, v_loss.item(), pg_loss.item(), entropy_loss.item())

        # Clock
        update_end_time = time.time()
        print("completed with time", math.floor(update_end_time-update_start_time), "seconds.")
